		...
		<div class="d-flex mt-5">
			...
			<c:forEach items="${photo.content}" var="photo">
				<div class="card ml-1">
					...
					<div class="card-body">
						<h5 class="card-title">
							<a href="/photo/${photo.id}">${photo.title}</a>
						</h5>
	...
	<ul class="pagination justify-content-center">
		...
		<c:choose>
			...
			<c:when test="${nowPage == startPage}">
				<li class="page-item disabled"><a class="page-link"
					href="/photo?page=${startPage - 1}&keyword=${keyword}" tabindex="-1">이전</a></li>
			</c:when>
			<c:otherwise>
		...
		<c:forEach begin="${startPage}" end="${endPage}" var="i">
			<c:choose>
				<c:when test="${nowPage == i}">
					<li class="page-item disabled"><a class="page-link"
						href="/photo?page=${i-1}&keyword=${keyword}">${i}</a></li>
				</c:when>
				<c:otherwise>
					<li class="page-item"><a class="page-link"
						href="/photo?page=${i-1}&keyword=${keyword}">${i}</a></li>
		...
		<c:choose>
			<c:when test="${endPage == nowPage}">
				<li class="page-item disabled"><a class="page-link"
					href="/photo?page=${endPage - 1}&keyword=${keyword}">다음</a></li>
			</c:when>
			<c:otherwise>
				<li class="page-item"><a class="page-link"
					href="/photo?page=${endPage - 1}&keyword=${keyword}">다음</a></li>
